...
class HankelDMD(DMDBase):
    ...
    “”“汉克尔动态模式分解:param
    svd_rank：截断的秩;如果为 0，则该方法计算最佳秩并将其用于截断;如果为正，该方法使用截断的参数;如果在 0 到 1 之间浮动，则秩是达到“svd_rank”指定的“能量”所需的最大奇异值的数量;如果为 -1，则该方法不计算截断。：类型 svd_rank：int 或 float :
    p aram int tlsq_rank：秩截断计算总最小二乘。默认值为 0，表示不截断。
    :p aram bool exact：用于计算精确 DMD 或投影 DMD 的标志。默认值为 False。
    :p aram opt：用于控制DMD模式幅度计算的参数。请参阅 ：class：'DMDBase'。默认值为 False。
    ：type opt： bool 或 int
    :p aram rescale_mode：比例 Atilde，如 10.1016j.jneumeth.2015.10.010（第 2.4 节）所示，然后计算其特征分解。无表示不重新缩放，“自动”表示使用单个值自动重新缩放，否则缩放因子。
    ：type rescale_mode： {'auto'} or None or numpy.ndarray :p aram bool forward_backward： 如果为 True，则像在 fbDMD 中一样计算低秩运算符（参考：https：arxiv.orgabs1507.02264）。默认值为 False。:
    p aram int d：输入快照空间维度的新顺序。默认值为 1。
    :p aram sorted_eigs：如果“sorted_eigs='abs'”，则按大小对特征值（和模式动力学）进行排序，如果“sorted_eigs='real'”，则按实部（然后按虚部打破联系）对特征值（和模式动力学）进行排序。默认值：假。
    ：type sorted_eigs： {'real'， 'abs'} or False
    :p aram reconstruction_method：由于HankelDMD的构思方式，用于从可用的多个版本中重建动态系统快照的方法。
    如果选择“first”（默认），则选择第一个可用版本（即最接近增强矩阵中第 0 行的版本）。
    如果“均值”，我们计算元素均值。如果 'reconstruction_method' 是一个浮点值数组，我们使用给定的值作为权重来计算加权平均值（对于每个快照）（权重的数量必须等于 'd'）。
    ：类型 reconstruction_method： {'first'， 'mean'} 或类似数组的 “””
    """
